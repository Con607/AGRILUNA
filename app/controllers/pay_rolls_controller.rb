class PayRollsController < ApplicationController
  ...
  def index
    if params[:search]
      ...
      @show_pay_rolls = true
      @greenhouse = Greenhouse.where(greenhouse_id: greenhouse_id)
      @cycle = Cycle.where(greenhouse_id: greenhouse_id).active.first
      from_date = @cycle.start_date
      to_date = @cycle.end_date
      @pay_rolls = PayRoll.where(greenhouse_id: greenhouse_id).where('start_date >= ?', from_date).where('end_date <= ?', to_date).order(:start_date)
